export interface ServiceWorkerFactoryOptions {
  cacheName: string;
  /** Paths relative to the SW scope (no leading slash required). */
  assets: string[];
}

/**
 * Generate a classic service-worker script string.
 * Write it to `sw.js` (or serve dynamically) — asset list is caller-owned.
 */
export function createServiceWorkerSource(options: ServiceWorkerFactoryOptions): string {
  ...
  return `/* generated by @krwg/palimpsest */
const CACHE = ${cache};
